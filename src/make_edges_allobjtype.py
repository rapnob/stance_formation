...
nrows = args.nrows
edges=pd.read_csv("/data/str01_03/twitter/hisamits/data/youtube/2021-06to08_RMQ_tlg/edges/2021-06-1-10_edges.csv", nrows=nrows, dtype=str)
...
for span in tqdm(spans):
    ...
    edges_sep = pd.read_csv("/data/str01_03/twitter/hisamits/data/youtube/"+dir_+"/edges/2021-"+span+"_edges.csv", nrows=nrows, dtype=str)
    edges_sep=edges_sep[edges_sep['source'].isin(selected_users.keys())].copy()
    print(edges_sep.dtypes)
